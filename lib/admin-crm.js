...
const SECRET_ANSWER_HASH_VERSION = 's2';
const SAFE_RESUME_MIME_TYPES = new Set([
  'application/pdf',
  'application/msword',
...
const SAFE_PROFILE_PICTURE_MIME_TYPES = new Set([
  'image/jpeg',
  'image/jpg',
  'image/png',
  'image/webp',
...
const REQUEST_EVENT_TYPES_SET = new Set(['submitted', 'approved', 'scheduled', 'declined', 'finalized', 'revoked']);
...
function getSystemAdminConfigIssue(usernameRaw) {
  const username = normalizeUsername(usernameRaw);
  if (!isSystemAdminUsername(username)) {
    return '';
  }

  const configuredPassword = getSystemAdminPassword();
  if (!configuredPassword) {
    return 'Admin environment credentials are missing. Set ADMIN_PASSWORD and redeploy.';
  }

  if (configuredPassword === 'change-me-StrongPass123') {
    return 'Admin environment credentials still use the placeholder ADMIN_PASSWORD value. Set the real password and redeploy.';
  }
...
function hashSecretAnswer(value) {
  ...
  const hashed = crypto.scryptSync(normalized, salt, 64).toString('hex');
  return `${SECRET_ANSWER_HASH_VERSION}:${salt}:${hashed}`;
}

function verifySecretAnswer(value, storedHash) {
  const normalized = normalizeSecretAnswer(value);
  const stored = sanitizeInlineText(storedHash || '', 400);
  ...
  if (stored.startsWith(`${SECRET_ANSWER_HASH_VERSION}:`)) {
    const [, salt, hashed] = stored.split(':', 3);
    if (!salt || !hashed) {
      return {
        matches: false,
        upgradedHash: '',
    ...
    const attempt = crypto.scryptSync(normalized, salt, 64).toString('hex');
    return {
      matches: timingSafeCompare(attempt, hashed),
      upgradedHash: '',
    };
  }
  ...
  const legacyHash = crypto
    ...
    .digest('hex');
  const matches = timingSafeCompare(legacyHash, stored);

  return {
    matches,
    upgradedHash: matches ? hashSecretAnswer(normalized) : '',
  };
}
...
function resolveSystemAdminAliases(primaryUsername, aliasesRaw) {
  ...
  const configuredAliases = typeof aliasesRaw === 'string' && aliasesRaw.trim()
    ...
    : [];
  const builtInAliases = ['admin', 'aoas'];

  builtInAliases.forEach((entry) => {
...
function isSelfSignupEnabled() {
  const raw = sanitizeInlineText(process.env.ADMIN_SELF_SIGNUP_ENABLED || 'false', 10).toLowerCase();
...
function mapAccountRecord(record) {
  ...
  const secretAnswerHashes = [
    sanitizeInlineText(record.secret_answer_1_hash || '', 400),
    sanitizeInlineText(record.secret_answer_2_hash || '', 400),
    sanitizeInlineText(record.secret_answer_3_hash || '', 400),
  ];
  ...
  return {
    ...
    failedResetAttempts: Math.max(0, Number.parseInt(String(record.failed_reset_attempts || 0), 10) || 0),
    resetLockedUntil: toIsoTimestamp(record.reset_locked_until),
...
function mapHiredProfileRecord(record) {
  return {
    ...
    updatedAt: toIsoTimestamp(record.updated_at),
    revokedAt: toIsoTimestamp(record.revoked_at),
    revokedBy: sanitizeInlineText(record.revoked_by || '', 80),
    revokedReason: sanitizeInlineText(record.revoked_reason || '', 240),
    revokeNote: sanitizeMultilineText(record.revoke_note || '', 2000),
    createdBy: sanitizeInlineText(record.created_by || '', 80),
...
function estimateBase64DecodedSize(base64Value) {
  const normalized = String(base64Value || '').replace(/=+$/g, '');
  if (!normalized) {
    return 0;
  }
...
function sanitizeResumePayload(value, existingResume = null) {
  ...
  if (!dataUrl || !parsed || dataUrl.length > MAX_RESUME_DATA_URL_LENGTH) {
    return existingResume || null;
  }

  const fileName = sanitizeInlineText(sourceObject.fileName || 'resume', 120) || 'resume';
  const mimeType = sanitizeInlineText(sourceObject.mimeType || parsed.mimeType, 120).toLowerCase() || parsed.mimeType;
  const size = Number.isFinite(Number(sourceObject.size)) ? Number(sourceObject.size) : null;
  const decodedSize = estimateBase64DecodedSize(parsed.base64);
  if (!SAFE_RESUME_MIME_TYPES.has(parsed.mimeType) || !SAFE_RESUME_MIME_TYPES.has(mimeType)) {
    return existingResume || null;
  }
  if (!decodedSize || decodedSize > MAX_RESUME_DATA_URL_LENGTH) {
    return existingResume || null;
  }
...
function sanitizeProfilePicturePayload(value, existingProfilePicture = null) {
  ...
  const parsed = parseDataUrlPayload(dataUrl);
  if (!dataUrl || !parsed || dataUrl.length > MAX_PROFILE_PICTURE_DATA_URL_LENGTH) {
    return existingProfilePicture || null;
  }
  ...
  const size = Number.isFinite(Number(sourceObject.size)) ? Number(sourceObject.size) : null;
  const decodedSize = estimateBase64DecodedSize(parsed.base64);
  if (!SAFE_PROFILE_PICTURE_MIME_TYPES.has(parsed.mimeType) || !SAFE_PROFILE_PICTURE_MIME_TYPES.has(mimeType)) {
    return existingProfilePicture || null;
  }
  if (!decodedSize || decodedSize > MAX_PROFILE_PICTURE_DATA_URL_LENGTH) {
    return existingProfilePicture || null;
  }
...
function normalizeSupabaseError(error, fallbackMessage = 'Database request failed.') {
  ...
  if (isMissingTableError(error)) {
    ...
    wrapped.status = 500;
    wrapped.publicMessage = 'Admin service is temporarily unavailable. Please try again later.';
    return wrapped;
  }
  ...
  if (String(error.code || '') === '23505') {
    ...
    wrapped.status = 409;
    wrapped.publicMessage = 'A record with that value already exists.';
    return wrapped;
  }
  ...
  if (String(error.code || '') === '42501') {
    ...
    wrapped.status = 403;
    wrapped.publicMessage = 'Admin service configuration is invalid. Please contact support.';
    return wrapped;
  }
...
function serializeAccountForDb(account, options = {}) {
  ...
  return {
    ...
    failed_reset_attempts: Math.max(0, Number(account.failedResetAttempts || 0)),
    reset_locked_until: account.resetLockedUntil || null,
...
async function loadAccountRowByUsername(supabase, username) {
  let result = await runAccountQueryWithFallback((columns) => supabase
    .from(TABLES.ACCOUNTS)
    .select(columns)
  ...
  // Legacy safety: some historical rows may contain non-lowercase usernames.
  result = await runAccountQueryWithFallback((columns) => supabase
    .from(TABLES.ACCOUNTS)
    .select(columns)
...
function isLikelySelfSignedUpViewerAccount(account) {
  if (!account || account.role !== ROLES.VIEWER || account.isActive !== true) {
    return false;
  }

  const creator = sanitizeInlineText(account.createdBy || '', 80).toLowerCase();
  const username = sanitizeInlineText(account.username || '', 40).toLowerCase();
  return Boolean(username) && creator === username;
}

async function disableLikelySelfSignedUpViewerAccounts() {
  const supabase = getSupabase();
  const { data, error } = await runAccountQueryWithFallback((columns) => supabase
    .from(TABLES.ACCOUNTS)
    .select(columns)
    .eq('role', ROLES.VIEWER)
    .eq('is_active', true));
  ...
  const candidates = (data || [])
    ...
    .filter(isLikelySelfSignedUpViewerAccount);
  ...
  for (const account of candidates) {
    ...
    await updateAccountRow(supabase, account.id, updated);
    disabled.push({
      id: account.id,
      username: account.username,
    });
  ...
  if (disabled.length) {
    console.warn(`Security bootstrap disabled ${disabled.length} likely self-created viewer account(s): ${disabled.map((entry) => `${entry.username} (${entry.id})`).join(', ')}`);
  }
...
async function runBootstrap() {
  ...
  await disableLikelySelfSignedUpViewerAccounts();
}
...
function buildResetLockedOutError(lockedUntil) {
  const untilDate = new Date(lockedUntil || '');
  const minutesRemaining = Number.isNaN(untilDate.getTime())
    ? Math.ceil(RESET_LOCKOUT_MS / 60000)
    : Math.max(1, Math.ceil((untilDate.getTime() - Date.now()) / 60000));
  const error = new Error(`Recovery is temporarily locked. Try again in about ${minutesRemaining} minute(s).`);
  error.status = 429;
  error.publicMessage = 'Recovery is temporarily locked. Try again later.';
  return error;
}

function buildGenericRecoveryError() {
  const error = new Error('We could not verify that recovery request.');
  error.status = 401;
  error.publicMessage = 'We could not verify that recovery request.';
  return error;
}
...
function isResetLocked(account) {
  if (!account?.resetLockedUntil) {
    return false;
  }

  const lockedAt = Date.parse(account.resetLockedUntil);
  return Number.isFinite(lockedAt) && lockedAt > Date.now();
}
...
async function persistFailedResetAttempt(supabase, account) {
  const failedResetAttempts = Math.max(0, Number(account.failedResetAttempts || 0)) + 1;
  const shouldLock = failedResetAttempts >= RESET_LOCKOUT_THRESHOLD;
  const updatedAccount = {
    ...account,
    failedResetAttempts,
    resetLockedUntil: shouldLock ? addDurationToNow(RESET_LOCKOUT_MS) : '',
    updatedAt: nowIso(),
  };
...
async function persistSuccessfulResetVerification(supabase, account, secretAnswerHashes) {
  const updatedAccount = {
    ...account,
    secretAnswerHashes: Array.isArray(secretAnswerHashes) && secretAnswerHashes.length === SECRET_QUESTION_KEYS.length
      ? secretAnswerHashes
      : account.secretAnswerHashes,
    failedResetAttempts: 0,
    resetLockedUntil: '',
    updatedAt: nowIso(),
  };
...
async function authenticateUser(usernameRaw, passwordRaw) {
  ...
  const systemAdminConfigIssue = getSystemAdminConfigIssue(username);
  if (isSystemAdminUsername(username)) {
    if (systemAdminConfigIssue) {
      const error = new Error(systemAdminConfigIssue);
      error.status = 503;
      error.publicMessage = 'Admin sign-in is not configured correctly. Contact the site operator.';
      throw error;
    }

    if (password === getSystemAdminPassword()) {
...
async function resolveUserFromRequest(req) {
  ...
  const passwordChangedAt = Date.parse(account.passwordChangedAt || '');
  if (Number.isFinite(passwordChangedAt) && payload.issuedAt && payload.issuedAt < passwordChangedAt) {
    return null;
  }
...
function readSecretAnswersFromPayload(source) {
  ...
  if (hasOwn(source, 'secretAnswerCityOfBirth')) {
    answers.city_of_birth = source.secretAnswerCityOfBirth;
  } else if (hasOwn(fromMap, 'city_of_birth')) {
    answers.city_of_birth = fromMap.city_of_birth;
  } else if (hasOwn(fromMap, 'cityOfBirth')) {
    answers.city_of_birth = fromMap.cityOfBirth;
  }

  if (hasOwn(source, 'secretAnswerFirstSchool')) {
    answers.first_school = source.secretAnswerFirstSchool;
  } else if (hasOwn(fromMap, 'first_school')) {
    answers.first_school = fromMap.first_school;
  } else if (hasOwn(fromMap, 'firstSchool')) {
    answers.first_school = fromMap.firstSchool;
  }

  if (hasOwn(source, 'secretAnswerChildhoodNickname')) {
    answers.childhood_nickname = source.secretAnswerChildhoodNickname;
  } else if (hasOwn(fromMap, 'childhood_nickname')) {
    answers.childhood_nickname = fromMap.childhood_nickname;
  } else if (hasOwn(fromMap, 'childhoodNickname')) {
    answers.childhood_nickname = fromMap.childhoodNickname;
  }
...
function sanitizeSecretAnswerBundle(source, options = {}) {
  ...
  const hasAnyProvided = normalizedAnswers.some(Boolean);
...
function sanitizeAccountWritePayload(payload, options = {}) {
  ...
  const secretAnswerHashes = SECRET_QUESTION_KEYS.map((key, index) => {
    ...
    return sanitizeInlineText(existingSecretAnswerHashes[index] || '', 400);
  });
...
async function updateSubAccount(accountIdRaw, payload) {
  ...
  if (input.password) {
    ...
    updated.lockedUntil = '';
    updated.failedResetAttempts = 0;
    updated.resetLockedUntil = '';
  }
...
async function createPasswordResetRequest(payload, options = {}) {
  ...
  if (!row) {
    throw buildGenericRecoveryError();
  }

  const account = mapAccountRecord(row);
  if (isResetLocked(account)) {
    throw buildResetLockedOutError(account.resetLockedUntil);
  }

  if (!account.isActive || !account.hasSecretRecovery) {
    throw buildGenericRecoveryError();
  }
  ...
  const verificationResults = account.secretAnswerHashes.map((storedHash, index) => (
    verifySecretAnswer(secretBundle.normalizedAnswers[index], storedHash)
  ));
  const matches = verificationResults.every((result) => result.matches);

  if (!matches) {
    const updatedAccount = await persistFailedResetAttempt(supabase, account);
    if (isResetLocked(updatedAccount)) {
      throw buildResetLockedOutError(updatedAccount.resetLockedUntil);
    }
    throw buildGenericRecoveryError();
  }

  const requiresHashUpgrade = verificationResults.some((result) => result.matches && result.upgradedHash);
  const verifiedAccount = await persistSuccessfulResetVerification(
    supabase,
    account,
    requiresHashUpgrade ? secretBundle.hashes : account.secretAnswerHashes,
  );
  ...
  return {
    ...
    resetToken: rawToken,
    expiresAt,
    account: publicAccountView(verifiedAccount),
  };
}
...
async function revokeHiredProfile(requestIdRaw, hireIdRaw, payloadRaw, actor) {
  assertAdmin(actor);
  await ensureBootstrap();

  const requestId = sanitizeInlineText(requestIdRaw, 80);
  const hireId = sanitizeInlineText(hireIdRaw, 80);
  if (!requestId || !hireId) {
    const error = new Error('request id and hire id are required.');
    error.status = 400;
    throw error;
  }
  ...
  const existing = mapClientRequestRecord(existingData);
  if (!canAccessClientRequest(existing, actor)) {
    const error = new Error('You do not have access to revoke hires for this request.');
    error.status = 403;
    throw error;
  }
  ...
  if (hireError) {
    throw normalizeSupabaseError(hireError);
  }

  if (!hireRow) {
    const error = new Error('Hired profile record not found.');
    error.status = 404;
    throw error;
  }

  const hiredProfile = mapHiredProfileRecord(hireRow);
  if (hiredProfile.revokedAt) {
    const error = new Error('This hire is already revoked.');
    error.status = 409;
    throw error;
  }

  const revokeReason = sanitizeInlineText(
    payload.reason || payload.revokedReason || 'manual_review',
    240,
  );
  const revokeNote = sanitizeMultilineText(payload.note || payload.revokeNote || '', 2000);
  const now = nowIso();
  ...
  const { error: revokeError } = await supabase
    ...
    .update({
      revoked_at: now,
      revoked_by: sanitizeInlineText(actor?.username || SYSTEM_ADMIN_USERNAME, 80),
  ...
  if (revokeError) {
    throw normalizeSupabaseError(revokeError);
  }

  const { data: activeHireRows, error: activeHireError } = await supabase
    .from(TABLES.HIRED_PROFILES)
    .select(HIRED_PROFILE_SELECT_COLUMNS)
    .eq('request_id', requestId)
    .is('revoked_at', null)
  ...
  if (activeHireError) {
    throw normalizeSupabaseError(activeHireError);
  }

  const activeHires = (activeHireRows || []).map(mapHiredProfileRecord);
  const activeParticipantIds = activeHires
    .map((profile) => sanitizeInlineText(profile.participantId || '', 80))
    .filter(Boolean);
  const activeParticipantSet = new Set(activeParticipantIds);
  const requestSelected = Array.isArray(existing.selectedParticipantIds)
    ? existing.selectedParticipantIds.filter(Boolean)
    : [];
  const allSelectedStillActive = requestSelected.length > 0
    && requestSelected.every((participantId) => activeParticipantSet.has(participantId));
  const nextStatus = activeHires.length
    ? 'finalized'
    : (existing.interviewDateTime || existing.ceoMeetingDateTime ? 'scheduled' : 'approved');
  const nextFinalizedIds = activeHires.length ? activeParticipantIds : [];
  const nextHireMode = activeHires.length ? (allSelectedStillActive ? 'all' : 'manual') : '';
  const revocationSummary = [
    `Revoked ${hiredProfile.participantName || 'hired profile'}.`,
  ...
  const updatedRequest = await updateClientRequestRow(supabase, existing.id, {
    status: nextStatus,
    finalizedHireMode: nextHireMode,
    finalizedParticipantIds: nextFinalizedIds,
    finalizedAt: activeHires.length ? existing.finalizedAt || now : null,
...
module.exports = {
  ...
  __test: {
    getSystemAdminConfigIssue,
    readSecretAnswersFromPayload,
    sanitizeSecretAnswerBundle,
    sanitizeAccountWritePayload,
  },
};
